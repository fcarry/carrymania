...
def ask_question(request: QuestionRequest):
    ...
    5. Es muy importante que tu respuesta sea solo una consulta sql SELECT ejecutable, no le agregues mas texto como SQL al inicio ni ninguna otra cosa que no sea solo responder la consulta SQL ejecutable
    Context: {context}
    Question: {question}
    Helpful Answer:"""
    ...
    print(f"Consulta SQL generada (sin procesar): {query_sql}")
    query_sql = re.search(r"```sql(.*?)```", query_sql, re.DOTALL).group(1).strip()
